...
DistMatrix = Compile[{{pos1, _Real, 2}, {pos2, _Real, 2}}, Module[{i, j},
   Return[Table[Norm[Mod[Round[pos1[[i]] - pos2[[j]], 10^-8]^2, 1]], {i, Length@pos1}, {j, Length@pos2}]]
]]

SymmetryOpVectorField[file_, pos_, vec_] := Block[{originshift, CifData, CifFlags, xyzName, xyzStrData, xyzRotTranData, xyzTranslation, xyzRotData, field, newpos, newvec, difftable, diff, posmap, i, j}, 
  If[! FileExistsQ[file], Print["Error:" <> file <> " was not found in the working directory!"]; Abort[]];
  CifData = Import[file, "string"] <> "\n";
